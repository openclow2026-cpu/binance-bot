...
function runBacktest(strategyFn, data, params = {}, config = {}) {
    ...
    for (let i = warmup; i < data.length; i++) {
        ...
        if (signal === 'BUY' && !position) {
            ...
            const price = data[i].close;
            const size = (balance * orderSize / 100) / price;
            position = { type: 'SHORT', entryPrice: price, size, entryIndex: i };
            balance -= size * price * (1 + commission / 100);
        }
            ...
            const price = data[i].close;
            const proceeds = position.size * price * (1 - commission / 100);
            const pnl = (position.size * position.entryPrice * (1 + commission / 100)) - proceeds;
            balance += proceeds;
            trades.push({ ...position, exitPrice: price, pnl, exitIndex: i });
            position = null;
        }
        ...
        if (exitConfig.sl && position) {
            ...
            if ((position.type === 'LONG' && data[i].low <= sl) || (position.type === 'SHORT' && data[i].high >= sl)) {
                const price = (position.type === 'LONG') ? data[i].high : data[i].low;
                const proceeds = position.size * price * (1 - commission / 100);
                const pnl = (position.type === 'LONG') ? 
                    proceeds - (position.size * position.entryPrice * (1 + commission / 100)) :
                    (position.size * position.entryPrice * (1 + commission / 100)) - proceeds;
                balance += proceeds;
        ...
        if (exitConfig.tp && position) {
            ...
            if ((position.type === 'LONG' && data[i].high >= tp) || (position.type === 'SHORT' && data[i].low <= tp)) {
                const price = tp;
                const proceeds = position.size * price * (1 - commission / 100);
                const pnl = (position.type === 'LONG') ?
                    proceeds - (position.size * position.entryPrice * (1 + commission / 100)) :
